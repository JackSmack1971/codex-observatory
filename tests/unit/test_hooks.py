...
def test_fresh_and_phase2_to_phase3_migrations_match(tmp_path: Path) -> None:
    ...
    phase2.commit()
    migrate(phase2)
    assert [row[0] for row in fresh.execute("SELECT version FROM schema_migrations ORDER BY version")] == list(range(1, 11))
    assert [row[0] for row in phase2.execute("SELECT version FROM schema_migrations ORDER BY version")] == list(range(1, 11))
