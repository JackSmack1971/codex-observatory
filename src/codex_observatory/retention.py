...
_SELECTIONS: Final[Mapping[str, tuple[str, str, str]]] = MappingProxyType({
    ...
    "git_snapshot_paths": ("SELECT p.snapshot_observation_id || ':' || p.path AS identity, s.captured_at AS timestamp, NULL AS retention_class FROM git_snapshot_paths p JOIN git_snapshots s USING(snapshot_observation_id) ORDER BY p.snapshot_observation_id,p.path", "snapshot_observation_id,path", "hot"),
...
def plan_retention(connection: Any, config: RetentionConfig, *, evaluation_time: datetime | None = None) -> RetentionPlanV1:
    ...
    for table in PRUNABLE_HISTORY_ORDER:
        sql, _identity_column, kind = _SELECTIONS[table]
        eligible: list[str] = []
