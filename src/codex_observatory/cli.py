...
def _doctor() -> int:
    ...
    try:
        ...
        retention = retention_health(app_db, config.retention)
        checks.append({"name": "retention", "status": retention["status"], "detail": retention})
        try:
            import duckdb
    ...
    degraded = any(item["status"] in {"missing", "degraded", "disconnected", "connecting", "incompatible", "not_configured", "not_implemented", "blocking", "HOOKS_CONFIGURED_NOT_OBSERVED", "configured_not_observed", "GIT_DEGRADED", "ARCHIVE_DEGRADED", "ARCHIVE_FAILED", "DUCKDB_QUERY_FAILED", "DUCKDB_UNAVAILABLE", "RETENTION_BLOCKED", "RETENTION_DEGRADED", "RETENTION_FAILED"} for item in checks)
