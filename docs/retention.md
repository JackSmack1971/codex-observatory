...
Only candidate identities durably marked both `COVERED` and `planned_delete=1`
are deleted. Deletion follows `PRUNABLE_HISTORY_ORDER` (children before
parents), records each table's actual delete count, verifies table counts before
commit, and atomically marks the run `COMPLETED`. An execution failure rolls
back the destructive transaction and records a zero-delete `FAILED` audit.
Uncovered candidates remain live. A second run is safe and produces a completed
zero-delete audit when nothing else has become eligible.

SQLite foreign keys remain enabled. Covered events and Git snapshots with live
dependent rows are excluded from the planned set because Phase 5 archives do
not contain stable identities for those children. Raw rows have no cascading
relationship to canonical events: removing raw diagnostics cannot remove
